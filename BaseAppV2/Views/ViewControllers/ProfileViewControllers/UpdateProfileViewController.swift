...
/**
    ...
    intereaction from the user updating their profile.
*/
final class UpdateProfileViewController: BaseViewController {
    
    // MARK: - IBOutlets
    @IBOutlet fileprivate weak var firstNameTextField: BaseTextField!
    @IBOutlet fileprivate weak var lastNameTextField: BaseTextField!
    @IBOutlet fileprivate weak var profileImageView: BaseImageView!
...
fileprivate extension UpdateProfileViewController {
    @IBAction private func editProfileImageButtonTapped(_ sender: BaseButton) {
    ...
    @IBAction private func updateProfileButtonTapped(_ sender: BaseButton) {
        updateProfile()
    }
}
    ...
    fileprivate func setup() {
        ...
        viewModel.updateProfileError.bind { [weak self] (error: BaseError?) in
            ...
            if updateError.statusCode == 101 {
                ...
                strongSelf.dismissProgressHudWithMessage(updateError.errorDescription, iconType: .error, duration: nil)
            }
        }
        viewModel.updateProfileSuccess.bind { [weak self] (success: Bool) in
