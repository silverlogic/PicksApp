...
/**
    ...
    interaction from the change password view.
*/
final class ChangePasswordViewController: BaseViewController {
    
    // MARK: - IBOutlets
    @IBOutlet fileprivate weak var currentPasswordTextField: BaseTextField!
...
fileprivate extension ChangePasswordViewController {
    @IBAction private func changePasswordButtonTapped(_ sender: BaseButton) {
        changePassword()
    }
}
    ...
    fileprivate func setup() {
        ...
        viewModel.changePasswordError.bind { [weak self] (error: BaseError?) in
            ...
            if changePasswordError.statusCode == 101 {
                ...
                strongSelf.dismissProgressHudWithMessage(changePasswordError.errorDescription, iconType: .error, duration: nil)
            }
        }
        viewModel.changePasswordSuccess.bind { [weak self] (success: Bool) in
