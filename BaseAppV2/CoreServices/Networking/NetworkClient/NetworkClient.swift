...
final class NetworkClient: Raccoon.Client {
    ...
    /**
        Initializes an instance of `NetworkClient` with a given base url and
        a context to insert objects in to when working with Groot dependency.
     
        - Parameters:
            - baseUrl: A `String` representing the base url of all requests. When
                       enqueing requests, endpoints would be appended to the base
                       url.
            - managedObjectContext: A `NSManagedObjectContext` representing the
                                    object space that objects would be inserted
                                    or updated to.
    */
