...
extension String {
    ...
    /**
        Truncates a `String` to the length of number of characters and appends optional
        trailing string if longer.
    ...
    */
    func truncate(length: Int) -> String {
        if self.characters.count > length {
            return self.substring(to: self.index(self.startIndex, offsetBy: length)).appending(" ...")
        } else {
            return self
        }
    }
...
extension String {
    ...
    /**
        Generates a random string to prevent cross-site request forgery. Required for
        Linkedin OAuth.
    ...
    */
    static func randomStateString(_ length: Int) -> String {
        ...
        for _ in 0..<length {
            ...
            let letter = letters.characters[index]
