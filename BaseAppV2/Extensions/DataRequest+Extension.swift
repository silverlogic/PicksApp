...
extension DataRequest {
    ...
    /**
        Validates a response given from the server. It
        calculates if the response is succesful or not.
     
        - Postcondition: The type of request is returned based
                         on the status code of the response. A
                         status code between the range of two hundred
                         to three hundred would be `success`. Otherwise,
                         `failure`.
     
        - Returns: A `DataRequest` representing the type of
                   request.
    */
