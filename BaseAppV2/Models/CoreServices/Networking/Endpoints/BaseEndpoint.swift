...
/**
    A subprotocol of `Raccoon.Endpoint` for building
    endpoints.
*/
...
extension BaseEndpoint {
    func request(withBaseURL baseURL: String) -> DataRequest {
        ...
        return Alamofire.request(url, method: endpointInfo.requestMethod, parameters: endpointInfo.parameters, encoding: endpointInfo.parameterEncoding, headers: headers)
    }
}


/**
    A struct encapsulating all required info needed
    for building requests from an endpoint.
*/
struct BaseEndpointInfo {
    ...
    */
    init(path: String, requestMethod: HTTPMethod, parameters: Parameters?, parameterEncoding: ParameterEncoding?, requiresAuthorization: Bool) {
        self.path = path
